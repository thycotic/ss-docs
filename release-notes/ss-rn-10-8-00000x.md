...
**Secret Server Version 10.7.00000x**
...
- 164884 - Added timing restricting to the refresh button on the "sign in as" pop-up window in the WPF. This is to limit the number of calls that can be made in a 10-second time frame from going back to SS to update the list of secrets.
- 164490 - Added a new feature to match URLs by exact path. This option looks at the domain value in the URL and will only list secrets that have an exact match. During web session recording, this option exactly matches the values in the yellow section. For example: `http://Company.Sub.Primary.Domain.TopDomain/subsite`. 
- 164490 - Improved support for sites that have top-level domains in the URL, such as sites that have ".co.uk" or ".online.com."
...
158361 - Personally Identifiable Information (PII) for user data in SS can now be anonymized to help companies meet compliance regulations for GDPR. 
...
- 159962 -  Improved performance when viewing the secret audit log, which has a large number of records, in the new UI.
- 164149 - Enhanced search performance in the new UI for instances with large amounts of data. Some enhancements automatically apply during the upgrade, but to fully take advantage of search optimization updates, customers should request assistance from Thycotic’s Support team to apply new advanced settings after upgrade.

### Security

==Moved most of this section to bug fixes==
...
- 154886 - Fixed a caching issue where discovery scanning sometimes did not find an existing organizational unit (OU) for SSC instances. ==Move to bugs?==
...
- 127410 - Fixed an issue were request access emails did not send to some approvers.
...
- 149450 - Added a new advanced configuration advanced setting, OffloadRunNowToDifferentNode. If true, offloads the “run now” commands to nodes with background worker enabled. ==I thought customers weren’t supposed to touch these settings? What’s the issue it fixed?== 
...
- 154540 - Fixed some areas that would still display errors when "zero information disclosure” was enabled.
...
- 159780 - Added a checkbox to the settings of the scheduled task and Windows service scanners to use NETBIOS name instead of UPN when matching domains. ==Bug fix?==
