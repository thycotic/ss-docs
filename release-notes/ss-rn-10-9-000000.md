...
- Fixed a memory leak appearing when the SSH port receives too many connection attempts in a short time. A third-party code library was not properly releasing memory used during failed connection attempts. Now, when an IP address makes too many (the default is over five) connection attempts over a specified time (the default is 30 minutes), that address is automatically blacklisted in the database, and future connections are denied. When this occurs, notifications appear in the  SS logs. The solution also prevents targeted DOS attacks on the SSH port.
