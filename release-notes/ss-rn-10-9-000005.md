...
**A critical issue has been discovered in Secret Server version 10.9.000032.**

**Thycotic encourages all customers to upgrade any 10.9.00032 installations to 10.9.000033 as soon as possible.** 

**Issue**
The issue occurs when managing local Groups, Users or Roles in the User Management section of the New UI. This covers pages with URLs starting with https://<secret server>/app/#/admin/user-management/
When editing lists the number of items in the list after editing will be limited to the number displayed in the UI. If the list previously contained additional items they will be removed when the list is edited.
Examples of this impact:

*	When editing a local group the number of users in the group will be limited to 30.
*	When editing the roles assigned to a group the list of roles will be limited to 30.
*	When editing the roles assignments for a user the list of roles will be limited to 10.
*	When editing the group memberships for a user the list of groups will be limited to 30.
*	When filtering a group member list by domain, then editing the members it will only show those domain members. Effectively removing members from other domains.

**Workarounds**
Use Classic UI when making changes to Group membership, Role assignment or editing a user’s Roles or Groups.
Secret Server 10.9.000032 will redirect attempts to edit users in the Classic UI to the New UI. 
To access the Classic UI for User management navigate directly to
https://<secret server>/users.aspx?legacyui=true

**Versions affected**
Secret Server 10.9.000032 is affected. 
Earlier versions of Secret Server are not affected.

Secret Server Cloud trials and new instances provisioned on or after 10 December 2020 are affected.
Existing Secret Server Cloud instances provisioned before 10 December 2020 are not affected.

**Resolution**
A customer impacted by this issue will need to manually restore their group and role assignments.
Secret Server audit logs will contain the necessary information, such as which users were removed from a group.
Any restoration should be done using either the Classic UI or the Secret Server API. 
...
function GroupUserAdd {
    ...
    $result = Invoke-RestMethod $url -Method GET -Headers $headers -ContentType "application/json"
    $maggieId = [USER ID]
    $maggie = $result.records | Where-Object {$_.userId -eq $maggieId}
    if($maggie) {
        $url = "$api/groups/[GROUP ID]/users/$maggieId"
        $result = Invoke-RestMethod $url -Method DELETE -Headers $headers -ContentType "application/json"
    }
    echo "----------------"
    echo "--GroupUserAdd--"
    echo "----------------"
    $url = "$api/groups/[GROUP ID]/users"
    echo "URL: $url"
    $args = @{UserId = $maggieId}|ConvertTo-Json
    $result = Invoke-RestMethod $url -Method POST -Body $args -Headers $headers -ContentType "application/json"
    if($result) {
        echo "---------------------------"
        echo "--GroupUserAdd Successful--"
        echo "---------------------------"
        echo $result
    } else {
        echo "ERROR: GroupUserAdd Failed"
        Exit
    }
}
...
function Execute {
    try {
        ...
        echo  $_.Exception.Response.StatusDescription
...
**Notes**
The script is for adding a single user to a group and the correct values for [SECRET SERVER URL], [GROUP ID], and [USER ID] should be inserted into the script as needed.
Actions taken by Thycotic
Secret Server version 10.9.000032 has been removed from our download sites.

**Further information**

This issue was discovered by Thycotic after the release of Secret Server version 10.9.000032.

A replacement version which resolves this issue will be published on December 14th, 2020 .
