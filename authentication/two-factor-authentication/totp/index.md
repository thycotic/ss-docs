...
Secret Server can also serve as an OTP generator, providing TOTP authentication for RPC and launchers. The soft token two-factor function in Secret Server is the “TOTP Authenticator” and you can use any application that uses the TOTP RFC6238 standard (details on the standard can be found [here](https://tools.ietf.org/html/rfc6238). An example of a TOTP application that works with Secret Server soft token two-factor authentication is Microsoft Authenticator.
