...
```powershell
$api = "https://%3CSecret Server URL>/winauthwebservices/api/v1"
...
try
{
  $api = "https://%3CSecret Server URL>/api/v1"
  $tokenRoute = "https://%3CSecret Server URL>/oauth2/token";
  ...
  { 
    Write-Host $[secret1.id](http://secret1.id)" - "$[secret1.name](http://secret1.name)" - "$secret1.folderId - $secret1.lastHeartBeatStatus
  }
  ...
  { 
    Write-Host $[secret.id](http://secret.id)" - "$secret.value
  }
...
{  
  ...
  $timestamp = Get-Date
  $[secret.name](http://secret.name) = "$timestamp"
  ...
  {
    if($item.fieldId -eq 87)
    {
      $item.itemValue = "theDomain"
    }
    if($item.fieldId -eq 90)
    {
      $item.itemValue = "myaccountname"
    }
    if($item.fieldId -eq 89)
    {
      $item.itemValue = "!@#ssword1"
    }
    if($item.fieldId -eq 88)
    {
  ...
  Write-Host $secret1
  Write-Host $[secret.id](http://secret.id)
}
...
try
{  
  $site = "https://%3CSecret Server URL>"
  $api = "$site/api/v1"
  ...
  $timestamp = Get-Date
  $secret.Active = $true
  $[secret.Name](http://secret.Name) = "Updated Secret$timestamp.Ticks"
...
{  
  ...
  Write-Host "Name: " = $[secret.name](http://secret.name)
...
$userCreateArgs = @{
    ...
    enabled = $true
    emailAddress = "newuser$[timestamp@v.com](mailto:timestamp@v.com)"
    twoFactor = $false
  ...
  Write-Host "New User ID : " $[user.id](http://user.id)
}
...
$folderId = $[folderAddResult.id](http://folderAddResult.id) 
...
if($[folderGetResult.id](http://folderGetResult.id) -eq $folderId)
{
  echo ""
  echo "-----------------------"
...
$folderStub.inheritPermissions = $false
$folderStub.inheritSecretPolicy = $false
$folderStub.parentFolderId = $[folderGetResult.id](http://folderGetResult.id)
$folderArgs = $folderStub | ConvertTo-Json
...
$childfolderId = $[folderChildAddResult.id](http://folderChildAddResult.id)
...
$folderPermissionId = $[permissionResults.id](http://permissionResults.id)
...
if($[permissionDeleteResult.id](http://permissionDeleteResult.id) -eq $folderPermissionId)
{
  echo ""
  echo "----------------------------------------"
